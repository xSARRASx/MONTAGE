"""Montage « sobre » : fond beige, cartes plein écran, visage dans un cadre arrondi, sous-titres épurés.

Usage : python3 composite_sobre.py <src.mp4> <dossier_3d> [images_de_test]
...
- Sur les plans visage, on recadre la vidéo au-dessus des anciens sous-titres (y < 76,6 %)
  et on la présente dans un cadre arrondi, avec de nouveaux sous-titres dessous.
"""
...
def draw_face(img, frames, t, ts, t0, lab, top=100, pill=None):
    label(img, t, t0 + 0.05, lab)
    soft_shadow(img, WIN_X, WIN_Y, WIN_W, WIN_H, WIN_R, 0.16, 30, 20)
    fw = face_window(frames, ts, top)
    sub = img[WIN_Y:WIN_Y + WIN_H, WIN_X:WIN_X + WIN_W]
    sub[:] = sub * (1 - WIN_MASK) + fw * WIN_MASK
    if pill:
        txt, pt = pill
        tt = o(pt)
        if t >= tt:
            k = ease_out_back((t - tt) / 0.4)
            im = text_img(((txt, INK),), 700, 34)
            pw, ph = im.shape[1] + 110, 76
            px, py = WIN_X + 36, WIN_Y + WIN_H - ph - 36
            fill_rrect(img, px, py + (1 - k) * 20, pw, ph, 38, CREAM, clamp(k))
            check_icon(img, px + 40, py + 38 + (1 - k) * 20, 20, SAGE, CREAM, clamp(k), clamp((t - tt - 0.1) / 0.3))
            blit(img, im, px + 72, py + 16 + (1 - k) * 20, clamp(k))
...
def card_fin(img, t, E):
    ...
    title(img, t, t0 + 0.12, [[('Lance ta', INK)], [('conciergerie.', SAGE)]], y=230)
    global MC_IMG
    if MC_IMG is not None:
        w, h = 940, 610
        op2, dy2 = appear(t, t0 + 0.25, 0.6)
        crop = MC_IMG[int((MC_IMG.shape[0] - h) / 2):int((MC_IMG.shape[0] - h) / 2) + h, int((MC_IMG.shape[1] - w) / 2):int((MC_IMG.shape[1] - w) / 2) + w]
        rgba = np.concatenate([crop, rrect_mask(w, h, 28)[..., None]], 2)
        soft_shadow(img, 70, 520 + dy2, w, h, 28, 0.22 * op2, 30, 22)
        blit(img, rgba, 70, 520 + dy2, op2)
    # bouton
    op3, dy3 = appear(t, t0 + 0.45)
    pulse = 1 + 0.025 * math.sin((t - t0) * 5) * clamp((t - t0 - 0.8) / 0.3)
    bw, bh = int(900 * pulse), int(128 * pulse)
    bx, by = W / 2 - bw / 2, 1240 + dy3 - (bh - 128) / 2
    soft_shadow(img, bx, by, bw, bh, 64, 0.25 * op3, 20, 14)
    fill_rrect(img, bx, by, bw, bh, 64, INK, op3)
    nudge = 10 * max(0, math.sin((t - t0) * 4)) * clamp((t - t0 - 0.8) / 0.3)
    put_text(img, [('VOIR LA MASTERCLASS', CREAM)], 800, 38, W / 2 - 40, by + bh / 2 - 28, op3, 'center', 2)
    ax, ay = int(W / 2 + 245 + nudge), int(by + bh / 2)
    lay = img.copy(); col = tuple(float(v) for v in SAGE_L)
    cv2.line(lay, (ax - 22, ay), (ax + 20, ay), col, 6, cv2.LINE_AA)
    cv2.line(lay, (ax + 4, ay - 16), (ax + 21, ay), col, 6, cv2.LINE_AA)
    cv2.line(lay, (ax + 4, ay + 16), (ax + 21, ay), col, 6, cv2.LINE_AA)
    img[:] = img * (1 - op3) + lay * op3
    op4, dy4 = appear(t, t0 + 0.7)
    put_text(img, [('100 % gratuite  ·  clique autour de la vidéo', MUTED)], 600, 32, W / 2, 1420 + dy4, op4, 'center')
...
def render_scene(sc, t, frames, E, chunks, thumbs):
    ...
    t0 = o(a)
    if kind == 'face_hook':
        draw_face(img, frames, t, ts, -1, 'LOCATION COURTE DURÉE', top=195)
    elif kind == 'face_10ans':
        draw_face(img, frames, t, ts, t0, "10 ANS D'EXPÉRIENCE", pill=("10 ans d'accompagnement", 9.15))
    elif kind == 'face_lcd':
        draw_face(img, frames, t, ts, t0, 'LOCATION COURTE DURÉE', pill=('Location courte durée', 16.6))
    elif kind == 'face_clic':
        draw_face(img, frames, t, ts, t0, 'MASTERCLASS GRATUITE', pill=('Clique autour de la vidéo', 24.98))
    elif kind == 'face_resultats':
        draw_face(img, frames, t, ts, t0, 'TON LANCEMENT', top=190, pill=('Résultats garantis', 32.74))
